...
class ActiveRecordModel
{
     ...
     * Get essential object properties.
     *
     * @return array top three most common tags.
     */
    public function topThreeTags()
    {
        $this->checkDb();
        return $this->db->connect()
                        ->select()
                        ->from($this->tableName)
                        ->executeFetchAll("SELECT tag, COUNT(tag) AS cnt FROM Tags GROUP BY tag ORDER BY cnt DESC LIMIT 3;");
    }
